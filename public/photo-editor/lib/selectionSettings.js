...
(function () {
  ...
  var selectionSettings = function () {
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="object-options">
          ...
          <button id="crop"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-crop" viewBox="0 0 16 16">
      ...
      $(
        ...
        // this.canvas.sendToBack(this.canvas.item(1));
        this.canvas.renderAll(), this.canvas.trigger("object:modified");
      });
      $(
      ...
      $(
        ...
        const cropImageContainer = document.querySelector(
        ...
        $(".free-crop").click(() => {
          ...
          cropperBox.style.borderRadius = "0";
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
    <div class="image-shape-section">
      <h4>Mask</h4>
      <label for="shape-selector">Select a shape:</label>
      <select class="form-select form-select-sm" id="shape-selector">
      ...
      <button class="btn btn-primary btn-sm mt-2 mb-2" id="apply-shape-mask">Apply</button>
      <hr>
    </div>
  `);

      // Function to apply a shape clipping mask to the selected image
      function applyShapeClippingMaskToImage(selectedImage, shape) {
        if (selectedImage && selectedImage.type === "image") {
          if (shape === "ellipse") {
            // Create an elliptical clipping mask
            const clippingMask = new fabric.Ellipse({
              rx: selectedImage.width / 2, // Adjust the size as needed
              ry: selectedImage.height / 2, // Adjust the size as needed
              originX: "center",
              originY: "center",
            });

            // Set the clipping mask to the selected image
            selectedImage.set({ clipPath: clippingMask });

            // Redraw the canvas
            _self.canvas.renderAll();
          } else if (shape === "circle") {
            const radius = selectedImage.width / 2;
            ...
            selectedImage.set({ clipPath: circle });
            _self.canvas.renderAll();
          } else if (shape === "triangle") {
            // Define the points for the triangle
            const trianglePoints = [
              { x: 0, y: -selectedImage.height / 2 }, // Top point
              { x: -selectedImage.width / 2, y: selectedImage.height / 2 }, // Bottom-left point
              { x: selectedImage.width / 2, y: selectedImage.height / 2 }, // Bottom-right point
            ];

            // Create a triangle clipping mask
            const clippingMask = new fabric.Polygon(trianglePoints, {
              fill: "transparent", // Transparent fill to allow the image to show
              originX: "center",
              originY: "center",
            });
            ...
            const starPoints = starPolygonPoints(
              5,
              selectedImage.width / 2,
              selectedImage.width / 4
            );

            // Create a star clipping mask
            const clippingMask = new fabric.Polygon(starPoints, {
              fill: "transparent", // Transparent fill to allow the image to show
              originX: "center",
              originY: "center",
            });
            ...
            function regularPolygonPoints(sideCount, radius) {
              ...
              for (var i = 0; i < sideCount; i++) {
                var x = radius * Math.cos(i * sweep);
                var y = radius * Math.sin(i * sweep);
                points.push({ x: x, y: y });
              }
              return points;
            }
            ...
            const hexagonPoints = regularPolygonPoints(
              6,
              selectedImage.width / 2
            ); // Set 6 sides for a hexagon

            // Create a hexagon clipping mask
            const clippingMask = new fabric.Polygon(hexagonPoints, {
              fill: "transparent", // Transparent fill to allow the image to show
              originX: "center",
              originY: "center",
            });
      ...
      // Handle applying the shape mask when the button is clicked
      $("#apply-shape-mask").click(function () {
        const selectedShape = $("#shape-selector").val();
        applyShapeClippingMaskToImage(_self.activeSelection, selectedShape);
      });
    })();
    ...
    <div class="custom-control-section">
      ...
      <button class="btn btn-primary btn-sm" id="edit" onclick="Edit()">Toggle control points</button>
      <hr>
    </div>
  `);
    })();
    ...
    // Edit function
    function Edit() {
      var selectedObject = _self.activeSelection;
      var canvas = _self_canvas;

      if (!selectedObject) return;

      if (selectedObject.type === "image") {
        // This is an image object, add custom controls and logic for images
        selectedObject.customControls = selectedObject.customControls || {};
        selectedObject.customControls.edit =
          !selectedObject.customControls.edit;

        if (selectedObject.customControls.edit) {
          // Customize controls and behavior for editing image objects
          // For example, add control points for resizing and other image-specific actions
          // ...

          _self_canvas.requestRenderAll();
        } else {
          // Restore default controls
          // Remove any custom controls or reset to default behavior
          // ...

          _self_canvas.requestRenderAll();
        }
      } else {
        // Handle other object types (e.g., shapes, text, etc.)
        // ...
