...
(function () {
  ...
  var defaultButtons = [
    ...
    },
    {
      ...
      icon: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-magic" viewBox="0 0 16 16">
    ...
    try {
      ...
      (() => {
        buttons.forEach((item) => {
          $(`${this.containerSelector} #toolbar .main-buttons`).append(
            `<button id="${item.name}" data-toggle="tooltip" data-placement="top" title="${item.title}">${item.icon}</button>`
          );
        });
      ...
      (() => {
        ...
        $(`${this.containerSelector}`).append(
          `<div class="floating-zoom-level-container" style="display: none;"></div>`
        );
        ...
        $(`${this.containerSelector} #toolbar .extended-buttons button`).click(
          function () {
            ...
            if (id === "save") {
              ...
              // reset the zoom before downloading the canvas image
              // so it will not affect the quality of the downloaded image
              var zoom = 1;
              typeof _self.applyZoom === "function" && _self.applyZoom(zoom);
